...
class App
{
    ...
    protected function registerRoute(string $method, mixed $callback, array $middlewares = [], bool $csrf = false): void
    {
        ...
        if (!Config::$DEBUG && is_string($cbKey)) {
            $cacheFile = Config::$DIR_PROJECT . '/app/cache/route_attribute_cache.php';
    ...
    protected function dispatch(): void
    {
        ...
        if ($method === 'GET' && isset($_GET['debug']) && Config::$DEBUG) {
            ...
            elseif (isset($_GET['fetch']))
                ...
                $this->render('app/debug', ['requests' => Debug::getRequests()]);
            exit;
        }
        ...
        if ($this->options['translate'] ?? Config::$TRANSLATE) {
            if (isset($_GET['set-lang'])) {
                ...
                if ($firstSegment !== '' && preg_match('/^[a-z]{2,3}(-[a-z]{2})?$/', $firstSegment)) {
                    $localeFile = Config::$DIR_PROJECT . "/app/locales/{$firstSegment}.php";
            ...
            if ($isValidRequest) {
                ...
                if ($reflection) {
                    ...
                    if (!isset(self::$diCache[$cbKey])) {
                        $cacheFile = Config::$DIR_PROJECT . '/app/cache/route_di_cache.php';
    ...
    protected function registerErrorHandler(): void
    {
        set_error_handler(function ($errno, $errstr, $errfile, $errline) {
            ...
            if ($this->isAjaxRequest()) {
                Response::JSON(['error' => true] + $details, 500);
                exit(1);
            }
...
HTML;
            ...
            Response::HTML(Template::templateLilaHTML($html), 500);
            exit(1);
        });
    }
