...
class Template
{
    ...
    {
        $appDir = rtrim(Config::$DIR_PROJECT, '/') . '/app';
...
HTML;
    ...
    public static function render(string $template, array $context = [], ?string $path = null): void
    {
        try {
            $isFrontend = self::isFrontendRequest();
            $twig = self::loadTwig($path ? Config::$DIR_PROJECT . $path : Config::$DIR_PROJECT . "/app/resources");

            $context['layout'] = $isFrontend ? "app/empty.twig" : "base.twig";
...
HTML;
    ...
    public static function getViteAssetsData(): array
    {
        ...
        if ($isDev) {
            ...
            $data['scripts'][] = ['src' => 'http://localhost:5173/main.js', 'type' => 'module'];
            return $data;
        }

        $manifestFile = Config::$DIR_PROJECT . '/app/cache/build_manifest.php';
        if (!file_exists($manifestFile))
            return $data;

        $manifest = require $manifestFile;
        $file = $manifest['main.js']['file'] ?? "main.js";
        $css = $manifest['main.js']['css'] ?? [];
