...
class Config extends Command
{
    ...
    public function cache(array $args): void
    {
        ...
        try {
            $dotenv = Dotenv::createMutable($dir . '/app');
            $envVars = $dotenv->load();

            $cacheFile = $dir . '/app/cache/env_cache.php';
            if (CoreConfig::saveCache($cacheFile, $envVars)) {
                $this->success("Configuration cached successfully at app/cache/env_cache.php");
                $this->info("Opcache will now handle configuration loading for better performance.");
            } else {
                $this->error("Failed to write cache file. Ensure app/ is writable.");
            }
        } catch (\Throwable $e) {
