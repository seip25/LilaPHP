...
(function () {
    ...
    function parseHash() {
        ...
        var q = {};
        raw.slice(i + 1).split('&').forEach(function (p) {
            var kv = p.split('=');
    ...
    function reactive(initial) {
        var subs = new Set();
        var raw = typeof initial === 'function' ? initial() : Object.assign({}, initial);

        function notify(key, value) {
            subs.forEach(function (fn) { fn(key, value); });
        }
        ...
        function proxyChild(value, parentKey) {
            ...
            return new Proxy(value, {
                get: function (t, k) {
                    var v = t[k];
                    if (Array.isArray(t) && typeof v === 'function' &&
                        ['push', 'pop', 'shift', 'unshift', 'splice', 'sort', 'reverse', 'fill'].indexOf(k) >= 0) {
                        return function () {
        ...
        return new Proxy(raw, {
            get: function (t, k) {
                if (k === '$subscribe') return function (fn) {
                    subs.add(fn);
    ...
    function morph(node, newHTML) {
        ...
        function walk(oldNode, newNode) {
            ...
            if (newNode.nodeType === 1) {
                ...
                if (!newNode.hasAttribute('checked') && oldNode.tagName === 'INPUT' && oldNode.type === 'checkbox') {
    ...
    function mount(target, config) {
        ...
        var initialState = typeof config.state === 'function' ? config.state() : (config.state || {});
        var state = reactive(initialState);
        var renderQueued = false;
        var mountCleanup = null;
        var pendingChanges = new Set();
        var templateFn = typeof def.template === 'function' ? def.template : function () { return def.template || ''; };

        function handleAction(e) {
            var n = e.target.closest('[data-action]');
            if (!n) return;
            var etype = n.getAttribute('data-event') || (n.tagName === 'FORM' ? 'submit' : 'click');
        ...
        function handleModel(e) {
            ...
            if (state[key] !== undefined) {
                ...
                else if (n.type === 'radio') { if (n.checked) state[key] = n.value; }
        ...
        function update(changedKeys) {
            if (!el) return;
            if (changedKeys) changedKeys.forEach(function (k) { pendingChanges.add(k); });
        ...
        var unsub = state.$subscribe(function (key) { update([key]); });
        update();
        ...
        var instance = {
            state: state,
            destroy: function () {
                if (typeof mountCleanup === 'function') { try { mountCleanup(); } catch (e) {} }
                if (def.onDestroy) { try { def.onDestroy(state); } catch (e) {} }
    ...
    async function handleRoute() {
        ...
        if (routeInstance) {
            try { routeInstance.destroy(); } catch (e) {}
            routeInstance = null;
        }

        var match = findRoute(h.path);
        if (!match) {
            outlet.innerHTML = '<div style="padding:2rem;text-align:center;color:#94a3b8"><h2>404</h2><p>Page not found</p></div>';
            return;
        }
        ...
        try {
            if (typeof handler === 'string') {
                ...
                outputHTML = (result && typeof result.then === 'function') ? await result : result;
            ...
            outputHTML = '<div style="padding:2rem;color:#f87171">Error loading page</div>';
        }
    ...
    function applyEnter() {
        ...
        setTimeout(function () { outlet.classList.remove('lila-active'); }, 200);
    }

    function start(selector, options) {
        injectStyles();
        outlet = typeof selector === 'string' ? document.querySelector(selector) : selector;
        if (!outlet) return;
    ...
    async function request(url, options) {
        ...
        if (opts.body) cfg.body = typeof opts.body === 'string' ? opts.body : JSON.stringify(opts.body);
    ...
    var Validator = (function() {
        var messages = {
            es: {
                required: function (f) { return 'El campo ' + f + ' es obligatorio'; },
                email: function (f) { return 'El campo ' + f + ' debe ser un email válido'; },
                min: function (f, n) { return 'El campo ' + f + ' debe tener al menos ' + n + ' caracteres'; },
                max: function (f, n) { return 'El campo ' + f + ' no puede tener más de ' + n + ' caracteres'; },
                equals: function (f, v) { return 'El campo ' + f + ' debe ser igual a ' + v; },
                pattern: function (f) { return 'El campo ' + f + ' no cumple el patrón requerido'; }
            },
            en: {
                required: function (f) { return f + ' is required'; },
                email: function (f) { return f + ' must be a valid email'; },
                min: function (f, n) { return f + ' must be at least ' + n + ' characters'; },
                max: function (f, n) { return f + ' must be at most ' + n + ' characters'; },
                equals: function (f, v) { return f + ' must equal ' + v; },
                pattern: function (f) { return f + ' does not match the required pattern'; }
            }
        };
        ...
        var rules = {
            ...
            equals: function (v, c) { return v === c; },
            matches: function (v, p) { return p.test(v); }
        };
            ...
            validate: function (data, schema, options) {
                ...
                for (var field in schema) {
                    ...
                    if (!rules.isEmpty(value)) {
                        ...
                        if (config.min !== undefined && !rules.isLength(value, config.min, undefined)) {
                            errors.push({ field: field, message: msg.min(field, config.min) });
                            success = false;
                        }
                        if (config.max !== undefined && !rules.isLength(value, undefined, config.max)) {
                            errors.push({ field: field, message: msg.max(field, config.max) });
                            success = false;
                        }
                        if (config.equals !== undefined && !rules.equals(value, config.equals)) {
                            errors.push({ field: field, message: msg.equals(field, config.equals) });
                            success = false;
                        }
